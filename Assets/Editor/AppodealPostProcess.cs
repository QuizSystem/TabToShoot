...
public class AppodealPostProcess : MonoBehaviour
{
	...
	private static void AddAdaptersDirectory(string path, PBXProject proj, string targetGuid)
	{
		...
		if (path.EndsWith (".framework", StringComparison.CurrentCultureIgnoreCase)) {
			proj.AddFileToBuild (targetGuid, proj.AddFile (realDstPath, path));
			string tmp = string.Format ("$(PROJECT_DIR)/{0}", path.Substring (0, path.LastIndexOf ("/")));
		...
		foreach (var filePath in Directory.GetFiles(realDstPath)) {
			...
			proj.AddFileToBuild (targetGuid, proj.AddFile (Path.Combine (realDstPath, fileName), Path.Combine (path, fileName), PBXSourceTree.Source));
			if(!libPathAdded && fileName.EndsWith(".a")){				
				proj.AddBuildProperty(targetGuid, "LIBRARY_SEARCH_PATHS", string.Format("$(PROJECT_DIR)/{0}",path));			
				libPathAdded = true;	
			}

			if(!headPathAdded && fileName.EndsWith(".h")){				
				proj.AddBuildProperty(targetGuid, "HEADER_SEARCH_PATHS", string.Format("$(PROJECT_DIR)/{0}",path));			
				headPathAdded = true;	
			}
		}
